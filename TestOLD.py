...
class SABREGUI(tk.Tk):
        ...
        def run_test_field():
            with self.task_lock:  # Ensure exclusive access to task resources
                try:
                    ...
                    self.plotting = True
                    for i in range(0, len(buf), sr // 10):
                    ...
                    if not self.stop_polarization:
                        messagebox.showerror("Error",
                            f"Failed to send polarization method to Ao0:\n{e}")
                finally:
                    if self.test_task:
                        try:
                            self.test_task.close()
    ...
    def set_voltage_to_zero(self):
        """Set the voltage to 0V on ao0 with proper cleanup"""
        try:
    ...
    def scram_experiment(self):
        """Emergency stop - immediate halt of all processes and return to initial state"""
        ...
        with self.task_lock:  # Ensure exclusive access during cleanup
            try:
                if self.test_task:
                    try:
                        if hasattr(self.test_task, '_handle') and self.test_task._handle:
                            self.test_task.stop()
                            self.test_task.close()
                    except:
                        pass
                ...
                for task in system.tasks:
                    try:
                        task_obj = nidaqmx.Task(task)
                        if hasattr(task_obj, '_handle') and task_obj._handle:
                            task_obj.stop()
                            task_obj.close()
                    except:
                        pass
        ...
        # 3. Create new tasks for reset operations
        try:
            # Reset voltage first
            ...
            try:
                ...
                if hasattr(ao_task, '_handle') and ao_task._handle:
                    ao_task.close()
            ...
            try:
                ...
                do_task.write([0, 0, 0, 0, 0, 0, 0, 0], auto_start=True)  # All channels set to LOW
            finally:
                if hasattr(do_task, '_handle') and do_task._handle:
                    do_task.close()

        except Exception as e:
            print(f"Emergency stop hardware reset failed: {e}")

        # 4. Reset GUI elements
        self.timer_label.config(text="00:00:000")
        self.state_label.config(text="State: Idle")
        self.reset_waveform_plot()
        
        # 5. Reset virtual panel if it exists
        if self.virtual_panel and self.virtual_panel.winfo_exists():
            self.virtual_panel.running = False
            for i in range(8):
                self.virtual_panel.update_hourglass_state(i, False)
            self.virtual_panel.load_config("Initial_State")

        # 6. Audio feedback in separate thread
        if self.audio_enabled.get():
            threading.Thread(target=lambda: winsound.Beep(2000, 1000), daemon=True).start()

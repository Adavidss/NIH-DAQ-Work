...
class ExperimentTimer:
    """Handles experiment timing with phase tracking and countdown functionality"""
    ...
    def start_experiment_timer(self, phases):
        """Start timer for complete experiment with multiple phases
        phases: list of (phase_name, duration) tuples
        """
        self.phases = phases
        self.total_duration = sum(duration for _, duration in phases if duration)
        self.elapsed_time = 0
        self.current_phase_index = 0
        self.is_running = True
        
        print(f"Starting experiment timer: {self.total_duration:.1f} seconds total")
        print(f"Phases: {[(name, f'{dur:.1f}s') for name, dur in phases if dur]}")
    ...
    def start_phase(self, phase_index):
        """Start a specific phase of the experiment"""
        if phase_index < len(self.phases):
            phase_name, duration = self.phases[phase_index]
            if duration:
                ...
                # Update GUI state
                if hasattr(self.gui, 'set_controls_state'):
                    self.gui.set_controls_state(phase_name)
                
                print(f"Starting phase {phase_index + 1}/{len(self.phases)}: {phase_name} ({duration:.1f}s)")
    ...
    def _update_display(self):
        ...
        if self.phase_start_time:
            ...
            total_elapsed = self.elapsed_time + phase_elapsed
            total_remaining = max(0, self.total_duration - total_elapsed)
            
            # Update timer labels
            self._update_timer_labels(total_remaining, phase_remaining)
        ...
        self.timer_job = self.gui.after(100, self._update_display)
        
    def _update_timer_labels(self, total_remaining, phase_remaining):
        """Update both total and phase timer displays"""
        # Update main timer (total remaining)
        if hasattr(self.gui, 'timer_label'):
            total_minutes = int(total_remaining // 60)
            total_seconds = int(total_remaining % 60)
            total_milliseconds = int((total_remaining % 1) * 1000)
            self.gui.timer_label.config(text=f"{total_minutes:02d}:{total_seconds:02d}:{total_milliseconds:03d}")
            
        # Update controls timer (phase remaining)  
        if hasattr(self.gui, 'controls_timer_var'):
            phase_minutes = int(phase_remaining // 60)
            phase_seconds = int(phase_remaining % 60) 
            phase_milliseconds = int((phase_remaining % 1) * 1000)
            self.gui.controls_timer_var.set(f"{phase_minutes:02d}:{phase_seconds:02d}.{phase_milliseconds:03d}")
            
    def _complete_current_phase(self):
        """Handle completion of current phase and advance to next"""
        if self.current_phase:
            print(f"Phase completed: {self.current_phase}")
            self.elapsed_time += self.phase_duration
            
        # Advance to next phase
        next_phase_index = self.current_phase_index + 1
        if next_phase_index < len(self.phases):
            self.start_phase(next_phase_index)
        else:
            self.complete_experiment()
            
    def complete_experiment(self):
        """Handle experiment completion"""
        print("Experiment timer completed")
        self.is_running = False
        
        # Set final timer displays to zero
        if hasattr(self.gui, 'timer_label'):
            self.gui.timer_label.config(text="00:00:000")
        if hasattr(self.gui, 'controls_timer_var'):
            self.gui.controls_timer_var.set("00:00.000")
        if hasattr(self.gui, 'set_controls_state'):
            self.gui.set_controls_state("Complete")
            
    def stop(self):
        """Stop the timer"""
        self.is_running = False
        if self.timer_job:
            self.gui.after_cancel(self.timer_job)
            self.timer_job = None
        print("Experiment timer stopped")
        
    def get_remaining_time(self):
        """Get total remaining time in seconds"""
        if not self.is_running or not self.phase_start_time:
            return 0
            
        current_time = time.time()
        phase_elapsed = current_time - self.phase_start_time
        total_elapsed = self.elapsed_time + phase_elapsed
        return max(0, self.total_duration - total_elapsed)
    
    def get_total_duration(self):
        """Get the total duration of all phases"""
    ...
    def get_current_phase(self):
        ...
        if self.current_phase:
            return {
                ...
                'remaining': max(0, self.phase_duration - (time.time() - self.phase_start_time)) if self.phase_start_time else 0
            }
        return None
...
class SABREGUI(VisualAspects):
    ...
    def __init__(self, master=None):
        ...
        self.experiment_timer = ExperimentTimer(self)
        self.controls_timer_widget = TimerWidget(self)
        self.controls_timer_widget.pack()
        self.scram = ScramController(self)
        self.setup_variables()

        # --- Notebook and Tabs Setup (must come first) ---
        ...
        self.status_label = tk.Label(self.status_timer_bar, textvariable=self.status_var, font=("Arial", 11, "bold"), fg="darkgreen", bg="#e0e0e0")
        self.status_label.pack(side="left", padx=(10, 20), pady=2)
        self.timer_label = tk.Label(self.status_timer_bar, text="00:00:00", font=("Courier", 14, "bold"), fg="#003366", bg="#e0e0e0", relief="sunken", padx=10, pady=2)
        self.timer_label.pack(side="right", padx=(0, 10), pady=2)
        # --- End Status/Timer Bar ---
    ...
    def setup_variables(self):
        ...
        self.current_preset_data = {}  # Store current preset parameters
          # Timer variables for experiment timing
        self.controls_timer_var = tk.StringVar(value="00:00.000")
        self.controls_state_var = tk.StringVar(value="State: Idle")
        self.app_launch_time = time.time()
        
        # Remove legacy countdown attributes - now handled by ExperimentTimer
    ...
    def _create_method_and_control_section(self, parent):
        ...
        # --- Add state and timer to controls section ---
        self.controls_state_var = tk.StringVar(value="State: Idle")
        self.controls_timer_var = tk.StringVar(value="00:00.000")
        controls_status_frame = tk.Frame(parent)
        controls_status_frame.pack(fill="x", padx=4, pady=(0, 2))
        ttk.Label(controls_status_frame, textvariable=self.controls_state_var, font=("Arial", 10, "bold"), foreground="blue").pack(side="left", padx=(0, 10))
        ttk.Label(controls_status_frame, textvariable=self.controls_timer_var, font=("Courier", 12, "bold"), foreground="#003366").pack(side="left")
        # --- End state/timer in controls section ---
        
        # Main experiment controls - Quadrant layout (2x2 grid)
        controls_frame = ttk.LabelFrame(parent, text="Control Buttons")
        controls_frame.pack(fill="x", padx=4, pady=(2, 2))
        controls_frame.configure(height=40)  # Reduce height
        ...
        # Create buttons in quadrant layout with less vertical padding
    ...
    def _create_quadrant_button(self, parent, text, color, command, row, col):
        ...
        button = tk.Button(parent, text=text, 
                          command=command,
                          font=('Arial', 8, 'bold'),  # smaller font for less height
                          relief="raised", bd=3,
                          width=4, height=1)  # height=1 is minimum, but smaller font reduces visual height
        # Set color scheme based on button type
        if text == "Activate":
            button.config(bg="#2E7D32", fg="white", activebackground="#2E7D32")
        elif text == "Start":
            button.config(bg="#1565C0", fg="white", activebackground="#1565C0")
        elif text == "Test Field":
            button.config(bg="#EF6C00", fg="white", activebackground="#EF6C00")
        elif text == "SCRAM":
            button.config(bg="#B71C1C", fg="white", activebackground="#B71C1C")
    ...
    def activate_experiment(self):
        ...
        missing_params = []
        required_fields = [
            ("Activation Time", getattr(self, 'activation_time_entry', None)),
            ("Temperature", self.entries.get("Temperature")),
            ("Flow Rate", self.entries.get("Flow Rate")),
            ("Pressure", self.entries.get("Pressure")),
            ("Injection Time", getattr(self, 'injection_time_entry', None)),
            ("Valve Control Timing", getattr(self, 'valve_time_entry', None)),
            ("Degassing Time", getattr(self, 'degassing_time_entry', None)),
            ("Bubbling Time", self.entries.get("Bubbling Time")),
        ...
        # Always proceed to start the experiment, even if missing params
    ...
    def run_activation_sequence(self):
        ...
        try:
            ...
            config = self.state_manager.load("Initial_State")
            if not config:
                messagebox.showerror("Error", "Failed to load initial state configuration")
                return
            if 'digital_outputs' in config:
                self.daq_controller.send_digital(config['digital_outputs'])
            if 'analog_outputs' in config:
                for channel, value in config['analog_outputs'].items():
                    self.daq_controller.send_analog({channel: value})
            ...
            # Use the new ExperimentTimer instead of old countdown methods
            valid_phases = [(state, duration) for state, duration in state_sequence if duration is not None]
            self.experiment_timer.start_experiment_timer(valid_phases)

            for state, duration in state_sequence:
                if not hasattr(self, 'running') or not self.running:
                    break
                config = self.state_manager.load(state)
                if config:
                    if 'digital_outputs' in config:
                        self.daq_controller.send_digital(config['digital_outputs'])
                    if 'analog_outputs' in config:
                    ...
                    if self.virtual_panel and self.virtual_panel.winfo_exists():
                      ...
                      # Wait for duration - let timer handle the countdown display
            ...
            self.state_manager.load("Initial_State")  # Always return to initial state
    ...
    def start_experiment(self):
        ...
        if not self.polarization_method_file:
            ...
            # Do not return; allow running for test/dev
    ...
    def run_bubbling_sequence(self):
        ...
        try:
            ...
            bubbling_time = method_dur if method_dur > 0 else self.get_value('bubbling_time_entry')            # Define complete experiment sequence with phases
            experiment_phases = [
                ("Valve_Stabilization", valve * 2),
                ("Bubbling", bubbling_time),
                ("Polarization_Method", method_dur),
            ...
            # Start the comprehensive experiment timer
            self.experiment_timer.start_experiment_timer(experiment_phases)
            
            # Start plotting without resetting the existing plot
            self.plotting = True
            
            # Load bubbling state with direct DAQ interaction
            config = self.state_manager.load("Bubbling_State_Initial")
            if not config:
            ...
            if 'analog_outputs' in config:
                for channel, value in config['analog_outputs'].items():
                    self.daq_controller.send_analog({channel: value})            
            # Run polarization method after initial valve stabilization
            def delayed_method():
                try:
            ...
            if hasattr(self, 'running') and self.running and not self.stop_polarization:
                ...
                for state, duration in remaining_states:
                    ...
                    if config:
                        ...
                        if self.virtual_panel and self.virtual_panel.winfo_exists():
                            self.virtual_panel.load_config_visual(state)
                          # Wait for duration - timer will handle the display updates
                        if duration is not None:
                            start_time = time.time()
            ...
            if not self.stop_polarization:  # Only if not already stopped
                self.state_manager.load("Initial_State")  # Return to initial state
                self.set_controls_state("Idle")
                # Update virtual panel if it exists
                if self.virtual_panel and self.virtual_panel.winfo_exists():
                    self.virtual_panel.load_config_visual("Initial_State")
    ...
    def run_polarization_method(self):
        ...
        self.set_controls_state("Polarization")
        with self.task_lock:  # Ensure exclusive access to task resources
            try:
                ...
                try:
                    ...
                    self.start_controls_countdown(method_duration)
                    ...
                    if not self.stop_polarization:
                        ...
                        self.state_manager.load("Transfer_Initial")
                        self.set_controls_state("Transfer")
                        ...
                        self.start_controls_countdown(transfer_time)
                        time.sleep(transfer_time)
                        # After transfer, proceed to recycle state
                        if not self.stop_polarization:
                            print("Transitioning to recycle state")
                            self.state_manager.load("Recycle")
                            self.set_controls_state("Recycle")
    ...
    def test_field(self):
        """Test the magnetic field functionality by sending the selected polarization method waveform to the DAQ."""
        try:
            if not self.polarization_method_file:
                messagebox.showerror("Error", "No polarization method selected for test field.")
                return

            # Load the selected method file
            ...
            with nidaqmx.Task() as task:
                ...
                writer.write_many_sample(buf)
                task.wait_until_done(timeout=len(buf) / sr + 2.0)
                task.write(0.0)  # Set to 0V after sending
            print("Test field: sent selected polarization method waveform to DAQ")
        except Exception as e:
            print(f"Error in test_field: {e}")
            messagebox.showerror("Error", f"Test field DAQ failed: {e}")
    ...
    def scram_experiment(self):
        ...
        try:
            ...
            if hasattr(self, 'controls_timer_var'):
                self.controls_timer_var.set("00:00.000")
    ...
    def load_config(self, state):
        """Load configuration for a given state and return the config dict"""
        try:
    ...
    def send_analog_signal(self, channel, value):
        """Send an analog output value to the specified channel on the DAQ."""
        try:
            import nidaqmx
            with nidaqmx.Task() as task:
                task.ao_channels.add_ao_voltage_chan(f"Dev1/{channel}", min_val=-10.0, max_val=10.0)
    ...
    def _write_analog_waveform(self, data, rate, continuous=False):
        ...
        try:
            ...
            if self.test_task is None:
                print("Error: test_task is not initialized.")
                return False
    ...
    def get_current_parameters(self):
        ...
        if self.parameter_section is not None:
    ...
    def refresh_preset_list(self):
        ...
        try:
            ...
            # Removed adv_preset_combobox references
            try:
                # Removed adv_preset_combobox references
                pass
            except:
            ...
            try:
                ...
                pass
            try:
                if hasattr(self, 'adv_preset_combobox') and getattr(self, 'adv_preset_combobox', None) is not None and self.adv_preset_combobox.winfo_exists():
    ...
    def _update_elapsed_timer(self):
        """
        Update self.timer_label with the hh:mm:ss elapsed since app start.
        Runs once each second via Tk.after().
        """
        if not hasattr(self, "timer_label") or not self.timer_label.winfo_exists():
            return  # Don't reschedule if label is gone
        elapsed = int(time.time() - self.app_launch_time)
        hours, remainder = divmod(elapsed, 3600)
        minutes, seconds = divmod(remainder, 60)
        self.timer_label.config(text=f"{hours:02d}:{minutes:02d}:{seconds:02d}")
        self.after(1000, self._update_elapsed_timer)

    def start_countdown(self, duration):
        """Legacy method for backward compatibility - now delegates to ExperimentTimer"""
        # For simple single-phase countdown
        phases = [("Countdown", duration)]
        self.experiment_timer.start_experiment_timer(phases)

    def _update_countdown(self):
        """Legacy method - timer updates now handled by ExperimentTimer"""
        pass

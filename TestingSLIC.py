...
DEFAULTS = {
    ...
    "df": 50,                        # Hz       – total sweep width (f_SLIC ± df/2)
...
class SLICSequenceControl(tk.Toplevel):
    ...
    def build_waveform(self):
        ...
        # ---- Improved adiabatic 90° pulse for stronger polarization ----
        pulse_i = np.arange(n_pulse, dtype=np.float64)        # 0 … n_pulse-1

        # Symmetric triangular B1 envelope: 0 → v_peak → 0
        env_pulse = v_peak * (1.0 - np.abs(2.0 * pulse_i / (n_pulse - 1) - 1.0))

        # Linear frequency sweep: start ABOVE resonance and sweep BELOW
        f_start = p["f_SLIC"] + 0.5 * p["df"]                # high edge
        f_end   = p["f_SLIC"] - 0.5 * p["df"]                # low edge
        f_pulse = f_start + (f_end - f_start) * (pulse_i / (n_pulse - 1))

        # Convert instantaneous frequency → cumulative phase (sample wise)
        delta_phase = 2.0 * np.pi * f_pulse / sr
        phase_pulse = np.cumsum(delta_phase) + phase_end_cw
        ...
        meta = {
            ...
            "pulse_shape":       "symmetric_triangular_high2low",

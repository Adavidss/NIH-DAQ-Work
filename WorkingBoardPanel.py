...
class SABREGUI(tk.Tk):
    ...
    def test_field(self):
        ...
        def run_test_field():
            ...
            try:
                ...
                # Set initial voltage
                self.test_task.write(initial_voltage)
                ...
                if not self.stop_polarization:  # Only show error if not intentionally stopped
                    messagebox.showerror("Error", f"Failed to send polarization method to Ao1: {e}")
            finally:
                if self.test_task and self.test_task.is_task_done():
    ...
    def scram_experiment(self):
        ...
        # Close the test task if it exists
        if self.test_task:
            try:
                self.test_task.write(0.0)
                self.test_task.close()
        ...
        try:
            system_tasks = nidaqmx.system.System.local().tasks
            for task_name in system_tasks:
                try:
        ...
        # Set both ao0 and ao1 to 0V immediately with new tasks
        try:
            with nidaqmx.Task() as task0, nidaqmx.Task() as task1:
                task0.ao_channels.add_ao_voltage_chan("Dev1/ao0", min_val=-10.0, max_val=10.0)
                task1.ao_channels.add_ao_voltage_chan("Dev1/ao1", min_val=-10.0, max_val=10.0)
